...
function fallbackNotes() {
  ...
  return `# Calen ${releaseVersion.releaseTag}\n\nRelease ${releaseVersion.releaseTag}.`;
}
...
function normalizeMarkdown(markdown) {
  ...
  if (!output.startsWith("#")) {
    output = `# Calen ${releaseVersion.releaseTag}\n\n${output}`;
  }
  return `${output.trim()}\n`;
}
...
function collectContext() {
  ...
  const repository = process.env.GITHUB_REPOSITORY?.trim() || "MiaTxxx/Calen";
...
function buildPrompt(context) {
  return [
    ...
    "- Start with exactly this H1: # Calen " + context.releaseTag,
