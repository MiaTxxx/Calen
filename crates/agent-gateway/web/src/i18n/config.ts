...
export const translations: Record<Locale, Record<string, string>> = {
  "zh-CN": {
    ...
    "settings.saveDraftsDesc": "切换会话、页面或重启浏览器后恢复尚未发送的文字和附件引用。",
    "settings.chatLayout": "聊天布局",
    "settings.chatLayoutDesc": "回复与输入区共用内容宽度，也可直接拖动输入框边缘。",
    ...
    "settings.remoteInfoBanner":
      "启用后，本地 Calen 将通过 gRPC 双向流连接云端 Gateway。你可以在浏览器中通过 WebUI 远程发送 Chat 消息、管理 Cron 任务和查看历史记录。所有工具执行仍在本地完成，远程端仅转发指令和结果。",
  ...
  "en-US": {
    ...
    "settings.saveDraftsDesc":
      "Restore unsent text and attachment references after switching conversations, pages, or restarting the browser.",
    "settings.chatLayout": "Chat layout",
    "settings.chatLayoutDesc":
      "Replies and composer share one width. Drag the composer edges to resize.",
    ...
    "settings.remoteInfoBanner":
      "When enabled, the local Calen connects to the cloud Gateway via gRPC bidirectional streaming. You can remotely send Chat messages, manage Cron tasks, and view history through the WebUI in your browser. All tool execution remains local — the remote end only relays commands and results.",
