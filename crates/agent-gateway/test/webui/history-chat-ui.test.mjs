...
test("WebUI transcript strips leaked DSML tool call markup from text and thinking", () => {
  const dsml = [
    ...
    '<||DSML|| parameter name="query">Calen DSML markup</||DSML|| parameter>',
    "</||DSML|| invoke>",
    "</||DSML|| tool_calls>",
  ].join("\n");
  const entries = chatUi.parseHistoryMessagesJson(JSON.stringify([
...
test("WebUI transcript hides provider-native web_search tool traces when hosted search exists", () => {
  const webSearchCall = {
    ...
    name: "web_search",
    arguments: { query: "Calen DeepSeek webui search" },
  };
  const entries = chatUi.parseHistoryMessagesJson(JSON.stringify([
    { role: "user", content: "search" },
    {
      ...
      content: [
        { type: "text", text: "searching" },
        {
          ...
          status: "completed",
          queries: ["Calen DeepSeek webui search"],
...
test("WebUI live transcript removes provider-native web_search when hosted search arrives later", () => {
  ...
  turn = applyEventToTurn(turn, {
    ...
    name: "web_search",
    arguments: { query: "Calen DeepSeek live search" },
    round: 1,
  });
  ...
  turn = applyEventToTurn(turn, {
    ...
    provider: "claude_code",
    status: "completed",
    queries: ["Calen DeepSeek live search"],
  ...
  const turn = reduceTurnEvents([
    {
      ...
      name: "WebSearch",
      arguments: { query: "Calen recovered search" },
      round: 1,
    },
    {
      type: "tool_result",
  ...
  const turn = reduceTurnEvents([
    {
      ...
      arguments: { query: "Calen DSML hidden search" },
      round: 1,
    },
  ]);
