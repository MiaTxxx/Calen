...
test("hosted search aggregation dedupes identical updates and separates completion from dispose", () => {
  ...
  const rawSearchEvent = {
    ...
    item: {
      ...
      id: "search-1",
      status: "in_progress",
      action: { query: "Calen hosted search" },
    },
  };
...
test("hosted search aggregation extracts structural Anthropic and Gemini search metadata", () => {
  ...
  anthropic.accept({
    ...
    content_block: {
      ...
      id: "toolu_1",
      name: "web_search",
      input: { query: "Calen Anthropic search" },
    },
  });
  anthropic.accept({
    type: "content_block_start",
  ...
  assert.deepEqual(anthropic.getBlocks()[0].queries, ["Calen Anthropic search"]);
  ...
  gemini.accept({
    candidates: [
      {
        groundingMetadata: {
          webSearchQueries: ["Calen Gemini search"],
          groundingChunks: [
            {
              web: {
  ...
  assert.deepEqual(gemini.getBlocks()[0].queries, ["Calen Gemini search"]);
...
test("hosted search aggregation accumulates an Anthropic query streamed as partial_json fragments", () => {
  ...
  anthropic.accept({
    type: "content_block_delta",
    index: 0,
    delta: { type: "input_json_delta", partial_json: 'ry": "Cal' },
  });
  assert.deepEqual(anthropic.getBlocks(), []);
  ...
  anthropic.accept({
    type: "content_block_delta",
    index: 0,
    delta: { type: "input_json_delta", partial_json: 'en incremental search"}' },
  });

  assert.deepEqual(anthropic.getBlocks()[0].queries, ["Calen incremental search"]);
...
test("hosted search aggregation extracts Anthropic citations_delta into the active search block", () => {
  ...
  anthropic.accept({
    ...
    content_block: {
      ...
      id: "toolu_cited",
      name: "web_search",
      input: { query: "Calen citation search" },
    },
  });
  anthropic.accept({
    type: "content_block_delta",
  ...
  assert.deepEqual(block.queries, ["Calen citation search"]);
  assert.deepEqual(
...
test("hosted search aggregation marks Anthropic web_search_tool_result_error as failed", () => {
  ...
  anthropic.accept({
    ...
    content_block: {
      ...
      id: "toolu_failed",
      name: "web_search",
      input: { query: "Calen failing search" },
    },
  });
  anthropic.accept({
    type: "content_block_start",
...
test("hosted search aggregation extracts OpenAI url_citation annotations and marks call completion", () => {
  ...
  openai.accept({
    ...
    item: {
      ...
      id: "search-annotated",
      status: "in_progress",
      action: { query: "Calen OpenAI search" },
    },
  });
  openai.accept({
  ...
  assert.deepEqual(block.queries, ["Calen OpenAI search"]);
  assert.deepEqual(
    block.sources.map((source) => ({ url: source.url, sourceType: source.sourceType })),
