...
Use this reference before installing or replacing Calen skills.
...
- Parse `name` and `description` before installing. For `README.md` without frontmatter, Calen derives the skill name from the directory and loads the full README when enabled.
...
- Do not manually edit Calen Settings. SkillsManager automatically enables newly installed skills for the current conversation when installation succeeds.

## After Install

Calen discovers skills through a scan of the fixed skills root. After an install, Calen refreshes discovery; if the UI still has stale state:
