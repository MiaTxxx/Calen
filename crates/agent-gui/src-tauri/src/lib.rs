...
macro_rules! app_invoke_handler {
    () => {
        tauri::generate_handler![
            ...
            commands::chat_history::chat_history_peek,
            ...
            commands::chat_history::chat_composer_draft_list,
            commands::chat_history::chat_composer_draft_get,
            commands::chat_history::chat_composer_draft_upsert,
            commands::chat_history::chat_composer_draft_delete,
            commands::chat_history::chat_composer_draft_clear,
            // Subagent store
            ...
            commands::system::system_pick_background_image,
            ...
            commands::system::system_validate_uploaded_file_references,
            ...
            commands::gateway::gateway_chat_context_reset_respond,
        ...
        .on_menu_event(move |app, event| match event.id().as_ref() {
            TRAY_SHOW_ID => {
                if let Err(error) = show_main_window(app) {
                    eprintln!("failed to show Calen window from tray: {error}");
                }
            }
            TRAY_QUIT_ID => {
                ...
                if running_count > 0 {
                    if let Err(error) = show_main_window(app) {
                        eprintln!(
                            "failed to show Calen window before terminal exit confirm: {error}"
                        );
                    }
            ...
            move |tray, event| match event {
                TrayIconEvent::DoubleClick {
                    ...
                    if let Err(error) = show_main_window(tray.app_handle()) {
                        eprintln!("failed to show Calen window from tray double-click: {error}");
                    }
                }
                TrayIconEvent::Click {
                    button: MouseButton::Left,
                    ...
                    if record_tray_left_click(&last_left_click_at) {
                        if let Err(error) = show_main_window(tray.app_handle()) {
                            eprintln!(
                                "failed to show Calen window from tray left double-click: {error}"
                            );
                        }
                    }
                }
...
pub fn run() {
    let automation_store = Arc::new(
        services::automation::AutomationStore::open()
            .expect("failed to initialize Calen automation store"),
    );
    let automation_scheduler = Arc::new(services::automation::AutomationScheduler::new(
        Arc::clone(&automation_store),
    ));
    let memory_store = Arc::new(
        services::memory::MemoryStore::open().expect("failed to initialize Calen memory store"),
    );
        ...
        .on_window_event(|window, event| {
            ...
            if let WindowEvent::CloseRequested { api, .. } = event {
                ...
                if let Err(error) = window.hide() {
                    eprintln!("failed to hide Calen window on close: {error}");
                }
            }
        })
    ...
    app.run(move |_app, event| match event {
        ...
        tauri::RunEvent::Reopen { .. } => {
            if let Err(error) = show_main_window(_app) {
                eprintln!("failed to show Calen window from dock reopen: {error}");
            }
        }
