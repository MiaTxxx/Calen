use calen_stock_sidecar_runtime::{normalize_node_launch_path, SidecarLaunch};
use chrono::Utc;
use regex::Regex;
...
fn resolve_sidecar_launch(app: &AppHandle) -> Result<SidecarLaunch, String> {
    if let Some(entry) = std::env::var_os("CALEN_STOCK_SIDECAR_ENTRY") {
        return SidecarLaunch::development(
            PathBuf::from(entry),
    ...
    if let Ok(resource_dir) = app.path().resource_dir() {
        ...
        if root.exists() {
            return SidecarLaunch::installed(&root);
        }
    }
    ...
    if !entry.is_file() {
        return Err(format!(
            "开发版股票入口不存在；请先构建 crates/stock-sidecar：{}",
            entry.display()
        ));
    }
    SidecarLaunch::development_at_root(root, entry, explicit_node)
}
