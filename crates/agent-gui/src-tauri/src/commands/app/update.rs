...
fn update_repository() -> String {
    std::env::var("CALEN_UPDATE_REPOSITORY")
        .ok()
        .or_else(|| option_env!("CALEN_UPDATE_REPOSITORY").map(str::to_string))
        .or_else(|| std::env::var("LIVEAGENT_UPDATE_REPOSITORY").ok())
...
fn updater_public_key() -> Option<String> {
    first_nonempty([
        std::env::var("CALEN_UPDATER_PUBLIC_KEY").ok(),
        option_env!("CALEN_UPDATER_PUBLIC_KEY").map(str::to_string),
        std::env::var("TAURI_UPDATER_PUBLIC_KEY").ok(),
        option_env!("TAURI_UPDATER_PUBLIC_KEY").map(str::to_string),
        std::env::var("LIVEAGENT_UPDATER_PUBLIC_KEY").ok(),
        option_env!("LIVEAGENT_UPDATER_PUBLIC_KEY").map(str::to_string),
    ])
}
...
async fn manifest_exists(client: &reqwest::Client, manifest_url: &str) -> Result<bool, String> {
    let response = client
        .head(manifest_url)
        .header(USER_AGENT, "Calen-Updater")
        .send()
        .await
...
fn updater_not_configured_response(
    app: &AppHandle,
    repository: String,
    channel: AppUpdateChannel,
) -> AppUpdateCheckResponse {
...
mod tests {
    ...
    fn feed_entry(tag_name: &str) -> ReleaseFeedEntry {
        ReleaseFeedEntry {
            ...
            title: Some(format!("Calen {tag_name}")),
            html_url: Some(format!(
                "https://github.com/MiaTxxx/Calen/releases/tag/{tag_name}"
            )),
...
<feed xmlns="http://www.w3.org/2005/Atom">
  <entry>
    ...
    <link rel="alternate" type="text/html" href="https://github.com/MiaTxxx/Calen/releases/tag/v0.1.2"/>
    <title>Calen v0.1.2</title>
  </entry>
</feed>"#,
        )
        ...
        assert_eq!(
            ...
            vec![ReleaseFeedEntry {
                ...
                title: Some("Calen v0.1.2".to_string()),
                html_url: Some(
                    "https://github.com/MiaTxxx/Calen/releases/tag/v0.1.2".to_string()
                ),
    ...
    fn prerelease_channel_can_select_prerelease_manifest() {
        ...
        assert_eq!(
            ...
            "https://github.com/MiaTxxx/Calen/releases/download/v0.1.2-beta.1/latest.json"
        );
    ...
    #[test]
    fn updater_public_key_selection_ignores_blank_values() {
        assert_eq!(
            first_nonempty([
                Some("  ".to_string()),
                None,
