...
<p align="center">
  <a href="https://github.com/MiaTxxx/Calen/releases/latest">最新版本：v1.1.7</a>
...
- 定时任务支持按分钟、小时或天设置自定义固定间隔，同时保留高级 Cron 表达式。
- Windows Agent 工具可以自由选择优先使用 Bash 或 PowerShell；所选 Shell 不可用时，自动模式会安全回退。
- 模型/API 流在尚未提交任何可见或可执行内容时遇到临时网络故障会自动重连；鉴权失败和额度不足仍会立即报错，不会盲目重试。

完整说明见 [v1.1.7 发布说明](docs/releases/v1.1.7.md)。
