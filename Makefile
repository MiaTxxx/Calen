...
DESKTOP_MACOS_APP_NAME ?= Calen
DESKTOP_MACOS_NOTARY_PROFILE ?= calen-notary
...
GATEWAY_DOCKER_IMAGE ?= calen-gateway:local
RELEASE_TAG ?=
...
github-release-main: check-github-release-tag
	...
	git tag -a "$(RELEASE_TAG)" -m "Calen $(RELEASE_TAG)"
...
proto:
	...
	protoc \
		...
		--go_opt=module=github.com/MiaTxxx/Calen/crates/agent-gateway \
		--go-grpc_out=$(AGENT_GATEWAY_DIR) \
		--go-grpc_opt=module=github.com/MiaTxxx/Calen/crates/agent-gateway \
...
gateway-build: proto webui
	CGO_ENABLED=0 go -C $(AGENT_GATEWAY_DIR) build -o bin/calen-gateway ./cmd/gateway
...
build-linux: proto webui
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go -C $(AGENT_GATEWAY_DIR) build -o bin/calen-gateway-linux-amd64 ./cmd/gateway

build-linux-amd: build-linux

build-linux-arm: proto webui
	CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go -C $(AGENT_GATEWAY_DIR) build -o bin/calen-gateway-linux-arm64 ./cmd/gateway

build-windows: proto webui
	CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go -C $(AGENT_GATEWAY_DIR) build -o bin/calen-gateway-windows-amd64.exe ./cmd/gateway
...
desktop-store-macos-notary-profile:
	@if [ -z "$(APPLE_ID)" ]; then echo "APPLE_ID is required. Example: APPLE_ID=name@example.com APPLE_TEAM_ID=<team-id> make desktop-store-macos-notary-profile"; exit 1; fi
	@if [ -z "$(APPLE_TEAM_ID)" ]; then echo "APPLE_TEAM_ID is required. Example: APPLE_ID=name@example.com APPLE_TEAM_ID=<team-id> make desktop-store-macos-notary-profile"; exit 1; fi
