...
RUN CGO_ENABLED=0 GOOS=${TARGETOS} GOARCH=${TARGETARCH} \
    go build -trimpath -ldflags="-s -w" -o /out/calen-gateway ./cmd/gateway
...
RUN useradd --system --uid 10001 --user-group --home-dir /nonexistent --shell /usr/sbin/nologin calen \
    && install -d -o calen -g calen -m 0700 /var/lib/calen

COPY --from=gateway-builder /out/calen-gateway /usr/local/bin/calen-gateway
...
ENTRYPOINT ["/usr/local/bin/calen-gateway"]
