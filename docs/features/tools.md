...
| WebUI Chat    | 受限间接执行 | WebUI 发 Chat Command 到 Gateway，模型执行仍发生在桌面端，但 Gateway 来源只注册公开股票研究工具和内存态 `TodoWrite`；文件、Shell、终端、Memory、Skills、MCP、Cron、SSH、Tunnel、自定义系统工具、子代理和 `StockPortfolioRead` 均不注册。 |
| Gateway       | 否           | Gateway 不执行业务工具，只转发 request/event 并维护 buffer。                                                                                                                                                                             |

该限制是模型能力门禁，不是结果字符串脱敏：远程来源在 `builtinRegistry` 组合阶段就拿不到本地主机 executor。普通桌面 Chat 与本地 Cron 继续使用原有工具 profile。
...
| silent extraction          | 隐式记忆提取阶段不直接让模型调用 mutation，而是解析 plan 后由 Calen 应用。 |
