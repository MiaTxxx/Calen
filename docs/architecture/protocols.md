...
远程 `chat.command` 的来源标记会一路传入桌面 builtin registry。Gateway 来源只允许公开股票研究和内存 Todo，不会注册本地文件、Shell、终端、Memory、Skills、MCP、Cron、SSH、Tunnel、自定义系统工具、子代理或资产账本读取能力；因此 Gateway relay 即使遭遇提示注入，也没有读取本地明文资产的工具句柄。
