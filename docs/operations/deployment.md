...
| 平台 | Runner | 产物 |
|---|---|---|
| macOS Intel | `macos-15-intel` | `Calen-vX.Y.Z-macOS-x64.dmg`，以及 updater 使用的 `.app.tar.gz` / `.sig`。 |
| macOS Apple Silicon | `macos-14` | `Calen-vX.Y.Z-macOS-aarch64.dmg`，以及 updater 使用的 `.app.tar.gz` / `.sig`。 |
